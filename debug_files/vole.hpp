...
namespace VOLE {
	...
	inline uint64_t AdjustTmpVoleT(uint64_t N_item, uint64_t requested_t);
	...
	*/
	...
	inline uint64_t AdjustTmpVoleT(uint64_t N_item, uint64_t requested_t) {
		if (N_item < 2) {
			throw std::invalid_argument("VOLE requires N_item >= 2");
		}
		if (N_item > std::numeric_limits<uint64_t>::max() / 2) {
			throw std::overflow_error("VOLE N_item is too large");
		}
		uint64_t max_t = (N_item * 2) / 3;
		uint64_t adjusted_t = std::min(requested_t, max_t);
		if (adjusted_t == 0) {
			throw std::invalid_argument("VOLE adjusted t is zero");
		}
		if (adjusted_t != requested_t) {
			std::cout << "VOLE adjust t from " << requested_t << " to "
			          << adjusted_t << " for N_item=" << N_item << std::endl;
		}
		return adjusted_t;
	} // Added by Martinmxq316 to prevent log2(0) in VOLE_A and VOLE_B by adjusting t to a small value when N_item is too small

	//(1.1) return vec_A and vec_C
	std::vector<block> VOLE_A(NetIO &A_io, uint64_t N_item, std::vector<block>& vec_C, uint64_t t){
		t = AdjustTmpVoleT(N_item, t);
